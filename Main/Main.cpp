...
void menu_y_opciones(int numerocuenta, float fondos, float nuevofondo, const char *nombre)
{
	...
	// OPCION/CASO 1 - ESTADO DE CUENTA
...
menu1:
	...
	do
	{
		...
		gotoxy(5, 2 + opcionSelec); // Coordenadas de la flechita de seleccion
		cout << "\t\t-->";// la flechita de seleccion
		...
		// UBICACION DE LAS DISTINTAS OPCIONES DEL MENU PRINCIPAL
		// CREA LA NUMERACION 1) 2) PARA CADA OPCION DEL MENU PRINCIPAL DE FORMA AUTOMATICA
		for (int i = 0; i < n; i++)
		{
			gotoxy(10, 3 + i);
			cout << "\t\t" << i + 1 << " ) " << opciones[i];
		...
		// switch para definir las acciones a mostrar dependiendo las teclas presionadas
		switch (tecla)
		{
		case UP:
			opcionSelec++;
		...
		case down:
			...
			break;
		case ENTER:
			menu = false; // el menu se convierte en false para que deje de mostrar el menu
			break;
		}
	...
	case 5:
	{
		...
		case 2:
		{
			...
			do
			{
				...
				// OPCIONES DE PAISES PARA ENVIAR LA REMESA
				// opciones del menu son en tipo vector o constante,
				for (int i = 0; i < n_r; i++)
				{
					gotoxy(10, 3 + i);
